...
;;; ---------------------------------------------------------------------
	...
	SECTION .text
        global  print_BigInt, add_BigInt, prod_BigInt, convert_string_to_num, free_BigInt, equal, init_BigInt
        extern  printf, malloc, free, atol
...
prod_BigInt:	
	...
	;; rdi and rsi are already in place
	call	order
	;; [rax] <= [rdx]
	mov	r13, rax
	mov	r12, [rax+b_size]
...
order:	
	;; given BigInt's with addresses in rdi and rsi
	;; return these addresses but in rax and rdx, such that rax <= rdx
	push 	rbp
	mov	rbp, rsp
	push	rbx			; save rbx, r12-r15
	push	r12
	...
	cmp	r12, r14
	jg	.switch
	;; now, we have equal sizes
	mov	rbx, r12		; qwords counter
	dec	rbx
.next_qword:
	cmp	rbx, 0
	jl	.end_qword
	mov	r8, qword [r13+8*rbx]
	mov	r9, qword [r15+8*rbx]
	cmp	r8, r9
	jb	.end
	cmp	r8, r9
	ja	.switch
	...
	jmp	.next_qword
.end_qword:
	;; at this point, both numbers are exactly equal
.switch:
	mov	rax, rsi
	mov	rdx, rdi	
.end:
...
free_BigInt:    
        ;; given a BigInt with address in rdi
        ;; return the result of freeing the BigInt
        push    rbp
        mov     rbp, rsp
	push 	rbx
	push	rax
